...
SCHEDULER.every '5s', :first_in => 0 do |job|
  ...
  if response.code.to_i == 200
    ...
    if data['status'].to_s == "idle"
      ...
      url='http://127.0.0.1:5000/get_display_stats?printer=gutenberg'
      response = Net::HTTP.get_response(url)
      if response.code.to_i == 200
        data = JSON.parse(response.body)
        job_name = data['filename']
        send_event('job_name', { text: job_name})

        remaining = Hash.new

        total_sec = data['details']['endtime'].to_i - data['details']['time_elapsed'].to_i
        ...
        total_sec = data['details']['time_elapsed'].to_i
